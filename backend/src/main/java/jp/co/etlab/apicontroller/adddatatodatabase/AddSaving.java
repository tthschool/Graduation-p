...
public class AddSaving implements HttpHandler {
    public void handle(HttpExchange exchange) throws IOException {
        if ("POST".equals(exchange.getRequestMethod())) {
            ...
            String saving_date = requestData.Getsavingdate();
            ...
            try {
                ...
                if (con != null) {
                    ...
                    if (Budget_id != -1) {
                        query = "insert into Savings ( description , amount , saving_date , budget_id   ) values (? , ?  , ? , ? ) ";
                        pstmt = con.prepareStatement(query);
                        pstmt.setString(1, Describle);
                        pstmt.setDouble(2, amount);
                        pstmt.setString(3, saving_date);
